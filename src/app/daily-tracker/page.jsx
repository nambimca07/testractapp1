...
export default function DailyTrackerForm() {
  ...
  //const categories = {
    //income: ['Salary', 'Investments', 'Freelance', 'Other'],
    //expenses: ['Food', 'Transportation', 'Utilities', 'Entertainment', 'Shopping', 'Other']
  //};
  ...
  const handleDeleteItem = (categoryId, type) => {
    const category = getCurrentDateData()[type === 'income' ? 'income' : 'expenses'].find(cat => cat.id === categoryId);
    if (category) {
      setDeleteConfirm({
        id: categoryId,
        name: category.name,
  ...
  const deleteCategory = (categoryId, type) => {
    setDailyTransactions((prev) => {
      ...
      if (type === 'income') {
        updatedData.income = dateData.income.filter(cat => cat.id !== categoryId);
      } else {
        updatedData.expenses = dateData.expenses.filter(cat => cat.id !== categoryId);
      }
  ...
  const handleRemarksEdit = (category, type) => {
    setShowRemarksPopup({ categoryId: category.id, type, categoryName: category.name });
    setTempRemarks(category.remarks || '');
    if (type === 'expense') {
      setTempUnit(category.unit || '');
      setTempUnitType(category.unitType || '');
  ...
  const handleSaveRemarks = () => {
    ...
    // Validate required fields for expense categories
    if (type === 'expense' && (tempUnit === '' || tempUnitType === '' || tempRate === '')) {
      setError('Please fill in all required fields for expense details');
      return;
    }
    ...
    setDailyTransactions((prev) => {
      ...
      if (type === 'income') {
        updatedData.income = dateData.income.map((cat) =>
          cat.id === categoryId ? { ...cat, remarks: tempRemarks.trim() } : cat
        );
      } else {
...
if (!user) {
  ...
  return (
    <DashboardLayout>
      <div className="max mx-auto py-8 px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-6 bg-white rounded-lg shadow p-6">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-4 sm:space-y-0">
            ...
            <div className="flex items-center space-x-6 text-sm">
              ...
              <div className="text-center">
                <p
                  className={`font-semibold text-lg ${summary.netAmount >= 0 ? "text-green-600" : "text-red-600"}`}
                >
              ...
              <div className="p-6 space-y-4 max-h-[600px] overflow-y-auto">
                ...
                {filteredIncomeCategories.map((category) => (
                  <div key={category.id} className="bg-gray-50 p-4 rounded-lg">
                      ...
                      <div className="flex items-center space-x-4">
                        <div className="flex items-center">
                          <input
                            ...
                            className="w-32 px-3 py-1 border border-gray-300 rounded-lg focus:border-green-500 focus:outline-none focus:ring-1 focus:ring-green-500"
                            placeholder="0.00"
                          />
                        ...
                        <div className="flex space-x-2">
                          <button
                            onClick={() => handleRemarksEdit(category, 'income')}
                          ...
                          <button
                            onClick={() => handleDeleteItem(category.id, 'income')}
                    ...
                    {category.remarks && (
                      <p className="text-sm text-gray-500">{category.remarks}</p>
                    )}
                  </div>
                ))}
                  ...
                  <div key={category.id} className="bg-gray-50 p-4 rounded-lg">
                      ...
                      <div className="flex items-center space-x-4">
                        <div className="flex items-center">
                          <input
                            ...
                            className="w-32 px-3 py-1 border border-gray-300 rounded-lg focus:border-red-500 focus:outline-none focus:ring-1 focus:ring-red-500"
                            placeholder="0.00"
                          />
                          <span className="ml-1 text-gray-600">₹</span>
                        </div>
                        <div className="flex space-x-2">
                          <button
                            onClick={() => handleRemarksEdit(category, 'expense')}
                            className="text-blue-500 hover:text-blue-700 p-1"
                            title="Edit details"
                          >
                            <i className="fas fa-edit"></i>
                          ...
                          <button
                            onClick={() => handleDeleteItem(category.id, 'expense')}
                            className="text-red-500 hover:text-red-700 p-1"
                            title="Delete category"
                          >
                            <i className="fas fa-trash"></i>
                    ...
                    {category.unit && (
                      <p className="text-sm text-gray-500">
                        Additional: {category.unit} {category.unitType} @ ₹{category.rate}
                      </p>
                    )}
                  </div>
                ))}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-lg w-96">
            <h3 className="text-lg font-semibold mb-4">
              Edit {showRemarksPopup.type === 'income' ? 'Income' : 'Expense'} Details
            </h3>
            <div className="space-y-4">
              <div>
              ...
              {showRemarksPopup.type === 'expense' && (
                <>
                  <div>
                    <label className="block text-sm font-medium text-gray-700">Additional Unit</label>
                    <input
                      type="number"
                      className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500"
                      value={tempUnit}
